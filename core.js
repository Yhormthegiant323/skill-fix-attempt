...
function enforceRobotDR(subset){
  if(!subset || !subset.id) return subset;
  const s = { ...subset };
  if(s.id === "protectron") s.drMod = 1;
  if(s.id === "mr_handy") s.drMod = -1;
  return s;
}

function getFinalDR(){
  const race = getRace?.();
  let dr = race?.drBase || 0;
  const subset = enforceRobotDR(getSubset?.(race));
  if(subset?.drMod) dr += subset.drMod;
  return dr;
}

function getFinalSpecial(stat){
  return state.special?.[stat] ?? 5;
}

function getFinalSkill(name){
  const base = state.skills?.[name] || 0;
  const tagged = state.tags?.includes(name) ? 3 : 0;

  const map = {
    guns:"AGI", energy:"PER", melee:"STR", unarmed:"STR",
    sneak:"AGI", lockpick:"PER", speech:"CHA", barter:"CHA",
    survival:"END", medicine:"INT", repair:"INT", science:"INT",
    explosives:"PER"
  };

  let spec = 0;
  const key = map[name.toLowerCase()];
  if(key && typeof scoreToMod === "function"){
    spec = scoreToMod(state.special?.[key] ?? 5);
  }

  return base + tagged + spec;
}
...
function renderSpecialRow(){
  ...
  el.innerHTML = stats.map(s=>{
    const v = getFinalSpecial(s);
    const m = scoreToMod(v);
    const sign = m>=0?"+":"";
    return `<div class="special-box">
      <div class="special-label">${s}</div>
      <div class="nixie-medium">${v}</div>
      <div class="special-mod">(${sign}${m})</div>
    </div>`;
  }).join("");
}

function renderSkillTotals(){
  const el = document.getElementById("skillTotals");
  if(!el || !state.skills) return;
  el.innerHTML = Object.keys(state.skills).map(k=>{
    const v = getFinalSkill(k);
    const sign = v>=0?"+":"";
    return `<div class="skill-row"><span>${k}</span><span>${sign}${v}</span></div>`;
  }).join("");
}
...
function renderSupportStats(el){
  ...
  el.innerHTML = `
    <div class="combat-grid">
      ...
      <div class="nixie-medium">DT ${s.dt}</div>
...
function renderSheet(){
  ...
  if(supEl) renderSupportStats(supEl);
  renderSpecialRow();
  renderSkillTotals();
}
 — ${subset} | Level ${lvl}`;
